...
import { fetchEvents, createEvent, updateEvent, deleteEvent, fetchRooms } from '../api';
...
export default function EventsPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [evRes, roomsRes] = await Promise.all([fetchEvents(), fetchRooms()]);
  ...
  const getAvailableDurations = () => {
    ...
    const roomEvents = events
      .filter(ev => ev.room_id === Number(form.room_id) && ev.id !== (editingEvent ? editingEvent.id : null))
      .map(ev => ({
    ...
    const nextEvent = roomEvents.filter(ev => ev.start >= startM).sort((a, b) => a.start - b.start)[0];
  ...
  const handleSubmit = async e => {
    ...
    if (!availableDurations.includes(Number(form.duration))) {
      setError('Invalid duration selected or does not fit in schedule');
      return;
    }
  ...
  return (
    ...
    <div className="container">
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: 20 }}>
        ...
        <select name="room_id" value={form.room_id} onChange={handleChange} required style={{ marginRight: 10 }}>
        ...
        <select
          ...
          style={{ marginRight: 10 }}
        >
          {availableDurations.length === 0 && <option>No available durations</option>}
        ...
        {editingEvent && <button onClick={cancelEdit} type="button" style={{ marginLeft: 10 }}>Cancel</button>}
      </form>
          ...
          <tbody>
            ...
            {events.map(ev => (
              <tr key={ev.id}>
                <td>{ev.id}</td>
                <td>{ev.title}</td>
                <td>{rooms.find(r => r.id === ev.room_id)?.name || 'Unknown'}</td>
                <td>{ev.start_time}</td>
                <td>{ev.end_time}</td>
                <td>{timeStringToMinutes(ev.end_time) - timeStringToMinutes(ev.start_time)} min</td>
                <td>
                  <button onClick={() => startEdit(ev)}>Edit</button>
                  <button onClick={() => handleDelete(ev.id)} style={{ marginLeft: 10 }}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
